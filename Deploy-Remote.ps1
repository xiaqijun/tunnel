...
# 执行远程部署命令
$command = "curl -fsSL https://raw.githubusercontent.com/xiaqijun/tunnel/main/deploy-to-server.sh | sudo bash"

ssh root@47.243.104.165 $command
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "建议：使用方式二（上传本地脚本部署）" -ForegroundColor Cyan
}
